...
//		var result = await db.rawQuery('SELECT * FROM $todoTable order by $colTitle ASC');
	...
	Future<int> updateTodo(Daily todo) async {
		var db = await this.database;
		var result = await db.update(todoTable, todo.toMap(), where: '$colId = ?', whereArgs: [todo.id]);
		return result;
	}

  	Future<int> updateTodoCompleted(Daily todo) async {
		var db = await this.database;
		var result = await db.update(todoTable, todo.toMap(), where: '$colId = ?', whereArgs: [todo.id]);
		return result;
	}
	...
	Future<List<Daily>> getTodoList() async {
		...
		List<Daily> todoList = List<Daily>();
		// For loop to create a 'todo List' from a 'Map List'
		for (int i = 0; i < count; i++) {
			todoList.add(Daily.fromMapObject(todoMapList[i]));
		}
